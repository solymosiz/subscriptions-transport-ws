...
describe('Client', function () {
    ...
    it('should take care of received keep alive', function (done) {
        ...
        var originalProcessReceivedData = subscriptionsClient.processReceivedData;
        subscriptionsClient.processReceivedData = function (receivedData) {
            var receivedDataParsed = JSON.parse(receivedData);
    ...
    it('should take care of invalid message received', function (done) {
        var subscriptionsClient = new client_1.SubscriptionClient("ws://localhost:" + RAW_TEST_PORT + "/");
        var originalProcessReceivedData = subscriptionsClient.processReceivedData;
        var receivedData = JSON.stringify({ type: 'invalid' });
        chai_1.expect(function () {
            originalProcessReceivedData.call(subscriptionsClient, receivedData);
        }).to.throw('Invalid message type!');
        done();
    });
    it('should throw if received data is not parseable', function (done) {
        var subscriptionsClient = new client_1.SubscriptionClient("ws://localhost:" + RAW_TEST_PORT + "/");
        var originalProcessReceivedData = subscriptionsClient.processReceivedData;
        var receivedData = 'invalid';
        chai_1.expect(function () {
            originalProcessReceivedData.call(subscriptionsClient, receivedData);
    ...
    it('should delete operation when receive a GQL_COMPLETE', function (done) {
        ...
        var originalProcessReceivedData = subscriptionsClient.processReceivedData;
        var receivedData = JSON.stringify({ id: 1, type: message_types_1.default.GQL_COMPLETE });
    ...
    it('should close the connection without sent connection terminate and reconnect', function (done) {
        ...
        var originalProcessReceivedData = subscriptionsClient.processReceivedData;
        subscriptionsClient.processReceivedData = function (dataReceived) {
...
describe('Server', function () {
    ...
    it('should trigger onConnect and return INIT_FAIL with error', function (done) {
        ...
        var originalProcessReceivedData = subscriptionsClient.processReceivedData;
        subscriptionsClient.processReceivedData = function (receivedData) {
            var messageData = JSON.parse(receivedData);
    ...
    it('should send a gql_data with errors message to client with invalid query', function (done) {
        var client1 = new client_1.SubscriptionClient("ws://localhost:" + TEST_PORT + "/");
        setTimeout(function () {
            client1.processReceivedData = function (receivedData) {
                var messageData = JSON.parse(receivedData);
    ...
    it('does not send more subscription data after client unsubscribes', function (done) {
        ...
        client4.processReceivedData = function (receivedData) {
            if (JSON.parse(receivedData).type === message_types_1.default.GQL_DATA) {
                chai_1.assert(false);
            }
        };
        sub = client4.request({
